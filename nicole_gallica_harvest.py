...
def main():
    print(f"Generating hash chain for: {CANONICAL}")
    layers = hash_chain_13(CANONICAL)
    
    print("Nicole's 13-layer hash chain:")
    for i, h in enumerate(layers, start=1):
        print(f"L{i}: {h}")
    
    # Save hash chain to JSON
    import json
    with open("nicole_layers.json", "w") as f:
        json.dump({"canonical": CANONICAL, "layers": [f"L{i}: {h}" for i, h in enumerate(layers, start=1)]}, f, indent=2)
    
    cql = '(gallica all "alchimie") and (dc.type any "monographie")'
    print(f"\nSearching Gallica with query: {cql}")
    sru_xml = sru_search(cql, max_records=12)
    arks = parse_arks_from_sru(sru_xml)
    
    print(f"Found {len(arks)} documents")
    ...
    for i, h in enumerate(layers, start=1):
        ...
        print(f"L{i}: Selected document {ark}, page {page_idx}, offset {off}")
    
    if not rows:
        print("No valid documents found - all ARKs failed to load")
        return
    
    with open("nicole_audit.csv", "w", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=list(rows[0].keys()))
        w.writeheader()
        w.writerows(rows)
    
    print(f"\nWrote nicole_audit.csv with deterministic pulls for L1–L13.")
    print(f"Generated {len(rows)} data points for Nicole")
